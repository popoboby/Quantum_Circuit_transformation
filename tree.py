...
class Node:
    ...
    def get_pertinent_swaps(self, coupling_map, _bit_indices, score_layer, dag): # 得到当前选择节点的最前层的相关交换门
        ...
        for node in self.front_layer: # 遍历最前层的节点
            for virtual in node.qargs: # 节点的门操纵 
                ...
                # neigbors = coupling_map.neighbors(physical)
                # neigbors
                # neigbors.sort()
                K = []
                ...
                # for physical_s in self.initial_layout:
                #     virtual_neighbor = physical_s.index 
                #     if virtual_neighbor == physical:
                #         continue
                #     if coupling_map.graph.has_edge(physical, virtual_neighbor):
                #         swap = sorted([virtual, virtual_neighbor], key=lambda q: _bit_indices[q])
    ...
    def execute_gates(self, coupling_map, dag, canonical_register): # 该节点一口气执行到front_layer不再更新为止
    ...
    def swap(self, swap, coupling_map, canonical_register, dag): # 执行完swap操作，并把front_layer推到最后一层，并计算其得分
        swap_node = DAGOpNode(op=SwapGate(), qargs=swap)
        # self._apply_gate(swap_node, canonical_register)
        self.initial_layout.swap(*swap)
        self.execute_gates(coupling_map, dag, canonical_register)
...
class Tree:
    def __init__(self, dag, width, depth, coupling_map, canonical_register, mapped_cir):
        ...
        new_node.execute_gates(coupling_map, dag, canonical_register)
    ...
    def expansion(self, coupling_map, _bit_indices, score_layer, dag, canonical_register):
        ...
        for node_id in self.exp_nodes: # 从遍历待拓展节点
            ...
            for swap, score, add_gate in zip(swaps, scores, add_gates):
                ...
                new_node.swap(swap, coupling_map, canonical_register, dag) # 该节点执行交换操作
        ...
        # if self.selec_count == 0:
        #     self.first_exp_layer_num = len(swap)
    ...
    def decision(self):
        if self.selec_count == self.depth:
            ...
            self.selec_count = 0
            # self.node_count = self.root_node + 1
            self.delete_nodes()
            # self.add_gates += 1 # 记录加入交换门的个数
